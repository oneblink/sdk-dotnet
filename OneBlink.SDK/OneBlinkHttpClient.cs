...
namespace OneBlink.SDK
{
    internal class OneBlinkHttpClient
    {
        ...
        public static void AddItemToQuery(System.Collections.Specialized.NameValueCollection query, string name, bool? item)
        {
            if (item.HasValue)
            {
                query[name] = item.ToString();
            }
        }
        public static void AddItemToQuery(System.Collections.Specialized.NameValueCollection query, string name, long? item)
        {
            if (item.HasValue)
            {
                query[name] = item.ToString();
        ...
        public static void AddItemToQuery(System.Collections.Specialized.NameValueCollection query, string name, DateTime? item)
        {
            if (item.HasValue)
            {
                query[name] = ((DateTime) item).ToString("yyyy-MM-ddTHH:mm:ssZ");
            }
        }
    }
}